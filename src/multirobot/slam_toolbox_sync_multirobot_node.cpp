...
int main(int argc, char ** argv)
{
  ...
  auto sync_node = std::make_shared<slam_toolbox::SynchronousSlamToolboxMultirobot>(options);
  sync_node->configure();
  sync_node->loadPoseGraphByParams();
  rclcpp::spin(sync_node->get_node_base_interface());     // TODO Make this multithreaded so each subscriber callback can run in parallel?
  rclcpp::shutdown();
  return 0;
}
